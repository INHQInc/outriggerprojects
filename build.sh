...
# Cache-busting — rewrite favorites.js and favorites.css references
# in HTML with a per-build hash so browsers always fetch the latest
# file. The hash is the first 8 chars of an md5 of the file content,
# so each change forces a fresh download but identical rebuilds reuse
# the same hash.
for demo_dir in public/demo public/demo-2; do
  ...
  if [ -f "$demo_dir/favorites.css" ]; then
    CSS_HASH=$(md5sum "$demo_dir/favorites.css" | cut -c1-8)
    find "$demo_dir" -name "*.html" -print0 | xargs -0 sed -i \
      "s|href=\"favorites.css\"|href=\"favorites.css?v=${CSS_HASH}\"|g"
    echo "Cache-busted favorites.css in $demo_dir (v=${CSS_HASH})"
  fi
done
